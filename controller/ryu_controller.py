...
from backend.config import FLOW_EPOCH_S, WHITELIST_IPS
from backend.mitigation.traffic_filter import RATE_LIMIT_PPS
try:
    from controller.ofp_rules import (
        INSTALL_DEDUP_TTL_S,
        delete_flow as _delete_flow,
        install_forward as _install_forward,
        install_drop as _install_drop,
        install_rate_limit_meter as _ofp_install_rate_limit_meter,
        build_drop_flow_mods,
        apply_proto_block_rule,
        build_clear_flow_mods,
        build_redirect_flow_mods,
    )
except (ImportError, ModuleNotFoundError):
    from ofp_rules import (
        INSTALL_DEDUP_TTL_S,
        delete_flow as _delete_flow,
        install_forward as _install_forward,
        install_drop as _install_drop,
        ...
        try:
            ...
            if _raw_ip:
                ...
                if _raw_eth:
                    ...
                    _install_forward(dp, parser, ofp, match, out_port, buf_id)
...
# Mitigation and clear install on ALL switches. An attacker can enter from any edge switch, so scoping to one leaves the others unprotected; clearing scoped to the last-seen switch leaves stale rules elsewhere, so clear is all-switch too.
    ...
    def _install_rate_limit_meter(self, dp, ofp, parser) -> None:
        _ofp_install_rate_limit_meter(dp, ofp, parser, _RATE_LIMIT_METER_ID, RATE_LIMIT_PPS)

    def _install_drop_rule(self, dp, ofp, parser, match, action: str, ttl) -> None:
        build_drop_flow_mods(dp, ofp, parser, match, action, ttl, _DROP_PRIORITY, _RATE_LIMIT_METER_ID)
    ...
    def _apply_proto_block(self, dp, ofp, parser, cmd: dict, dpid: int) -> None:
        ...
        if proto is None:
            return
        apply_proto_block_rule(dp, ofp, parser, proto, remove, dpid, self.logger)

    def _install_clear_rules(self, dp, ofp, parser, match) -> None:
        build_clear_flow_mods(dp, ofp, parser, match)
        _src = match.get("ipv4_src")
        if _src:
            self._forget_install(dp.id, _src)

    def _install_redirect_rule(self, dp, ofp, parser, match, redirect_to: str) -> None:
        build_redirect_flow_mods(dp, ofp, parser, match, redirect_to)

    # -- Helpers ------------------------------------------------------------

    def _flood(self, dp, ofp, parser, msg, in_port) -> None:
        self._send_packet_out(dp, ofp, parser, msg, in_port, [parser.OFPActionOutput(ofp.OFPP_FLOOD)])
    ...
    def _install_per_ip_meters(self, dp, ofp, parser, ip_list: list[str],
        ...
        for src_ip in ip_list:
            ...
            dp.send_msg(parser.OFPFlowMod(
                ...
                idle_timeout=INSTALL_DEDUP_TTL_S,
                hard_timeout=300,
                match=match, instructions=inst,
            ))

    def _remove_per_ip_meters(self, dp, ofp, parser) -> None:
        meter_ips = getattr(self, "_installed_meter_ips", None)
        if not meter_ips:
            return
        for src_ip, meter_id in list(meter_ips.items()):
            try:
                dp.send_msg(parser.OFPMeterMod(
