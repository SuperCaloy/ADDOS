/**
 * Expert Mode coordinator and live algorithmic metrics readout engine.
 * Orchestrates canvas visualizations, polling intervals, EventBus streaming, and diagnostic telemetry feeds.
 */

// Timers for periodic expert polling and ambient canvas particle generation.
let _expertPollTimer = null;
let _ambientTimer = null;

/**
 * Toggles the expert analytics mode between active and hidden states.
 * Synchronizes store flags, toggles CSS visibility classes, and starts or stops polling cycles.
 */
...
// Restores previously saved expert mode preferences upon initial DOM content load.
...
/**
 * Updates the network status indicator badge to reflect streaming or polling modes.
 * Displays an animated pulse dot during active SSE streaming or switches to subdued polling tags.
 */
...
/**
 * Activates expert mode data collection loops and visual rendering components.
 * Initiates telemetry polling, registers EventBus stream listeners, and starts canvas particle animations.
 */
...
/**
 * Halts expert mode data fetching loops and releases visual event listeners.
 * Clears recurring timers, unbinds EventBus event handlers, and stops canvas animation loops.
 */
...
/**
 * Fetches real-time expert telemetry from the backend API and dispatches metrics to UI components.
 * Updates ML panels, mitigation state machine views, canvas activity glows, and trend sparklines.
 */
...
/**
 * Handles incoming EventBus streaming telemetry payloads for immediate UI updates.
 * Updates entropy gauges, triggers particle animations on inference events, and logs mitigation actions.
 */
...
// Controller managing live metric statistics, trend sparklines, and terminal log feeds.
var ExpertMetrics = {
  /**
   * Builds the DOM layout for the live pipeline metric readout panel, sparkline SVGs, and terminal feed.
   * Initializes placeholders for throughput rates, entropy measures, anomaly ratios, and mitigation tallies.
   */
  init: function() {
  ...
  /**
   * Updates textual metric readouts across the live pipeline dashboard from incoming telemetry snapshots.
   * Formats packet rates, entropy variance, TEA verdict tags, prefilter spikes, and mitigation counts.
   */
  ...
  /**
   * Generates polyline coordinate data for the real-time detection trend chart.
   * Maps historical Isolation Forest scores and Random Forest confidence values to SVG coordinate points.
   */
  ...
  /**
   * Appends a new timestamped log message entry to the live diagnostic terminal.
   * Automatically scrolls to the bottom and trims older messages to stay within the maximum log buffer size.
   */
...
/**
 * Exports current live expert diagnostic telemetry as a downloadable JSON file.
 * Creates an in-memory blob URL and triggers programmatic download in the browser.
 */
