...
# benchmark import stays main-only: at module top the sys.path insert would
# make "topology" resolve to this file and break all later topology.* imports.
...
# campaign start times so stop posts carry a cutoff (ground-truth race safety)
...
# 1 core switch plus n_edge edge switches; hosts on a flat 10.0.0.x/24. Layout is fixed so the topology is identical every run: s1-s7 carry h1-h25, s8 carries the server only.
    ...
    # Fixed host-to-switch mapping, deterministic every run: h1-h25 on
    # s1-s7, h26 server alone on s8, h27 sinkhole on the core.
    _HOST_TO_SWITCH = {
        1: 1,  2: 1,  3: 1,  4: 1,
        5: 2,  6: 2,  7: 2,  8: 2,
        9: 3,  10: 3, 11: 3, 12: 3,
    ...
    _host_switch_map["h27"] = core.name

    return _net, _hosts, edge_switches
...
def _nsrun(host, cmd: str, wait: bool = False, return_proc: bool = False):
    # Run a command inside the host's netns/pid namespace, detached so a
    # SIGINT to this script does not kill the flood.
...
def _baseline_loop(host, stop_event: threading.Event, idle_host_ref: list) -> None:
    ...
    while not stop_event.is_set():
        ...
        # active phase, send every 3-7s for 45s (scaled by per-host multiplier)
        mult = _LEGIT_SLEEP_MULTIPLIERS.get(num, 1.0)
        end_active = time.time() + 45
...
def start_server() -> None:
    # raw L4 TCP and UDP listeners on h26
    server = net.get("h26")
    ...
    info(f"*** Raw L4 server started on h26 ({SERVER_IP}) TCP:80,443,8080 UDP:53,123,1900\n")
...
def _flush_flow_rules(prios, ips=None, proto_wildcard_pri=None,
    ...
    # Batched strict deletes, one shell chain per switch run concurrently;
    # exact-match strict deletes are required on the installed OVS 2.17.9.
    if net is None:
        return
    if ips is None:
...
def _notify_gt_stop_all(cutoff: float, retries: int = 3) -> None:
    # One batched ground-truth stop; the cutoff keeps campaigns started after
    # this stop began race-safe server-side.
...
def _stop_active_workers() -> None:
    # One attack campaign at a time: stop prior workers so floods never stack.
    ...
    if net is not None:
        _kill_all_attackers()
    # Every campaign starter runs this first, so stale per-IP protocol caches
    # from the previous run are flushed before the first new attack packet.
    _flush_proto_cache()
...
def _attacker_cycle_worker(num: int, stop_event: threading.Event,
    ...
    # Continuous flood until stop_event; delay overrides the per-host start
    # jitter so staged waves can time hosts into vector slots.
    h     = net.get(f"h{num}")
    if delay is None:
...
# Launch all attackers with sequential types via worker threads that monitor hping3 and keep the flood running until stop_all_attacks().
    ...
    info(f"*** Sustained DDoS (sequential: SYN -> UDP -> ICMP), all attackers -> {SERVER_IP}\n\n")
...
def launch_syn_flood_sustained(attacker_name="h16") -> None:
    global _mixed_stop_event, _campaign_threads
    _stop_active_workers()
    _mixed_stop_event.clear()
...
def launch_icmp_flood_sustained(attacker_name="h23") -> None:
    global _mixed_stop_event, _campaign_threads
    _stop_active_workers()
    _mixed_stop_event.clear()
    num = int(attacker_name[1:])
...
def launch_udp_flood_sustained(attacker_name="h20") -> None:
    global _mixed_stop_event, _campaign_threads
    _stop_active_workers()
    _mixed_stop_event.clear()
    num = int(attacker_name[1:])
...
# Bounded detection watch: for up to window_s after a wave start, poll backend evidence and poke any unseen-but-alive attacker so the attack cannot run invisible.
    ...
    while not stop.is_set() and now_fn() - start < window_s:
        payload = evidence_fn()
        if payload is not None and now_fn() - start >= _POKE_GRACE_S:
...
# Stagger each attacker by 100ms so OVS is not hit by all floods in the same millisecond, which causes switch disconnects.
    def _stress_worker(num: int) -> None:
        ...
        # _nsrun uses Popen and survives this call; h.cmd() would kill any
        # backgrounded child with its closing shell.
        _nsrun(h, cmd)
        info(f"    h{num} ({h.IP()}): stress flood started\n")

        # Watchdog: relaunch hping3 every 5s if it died on its own, until
        # stop_stress_test() sets the stop event.
...
def stop_all_attacks() -> None:
    ...
    # Set BOTH stop events FIRST, so stress watchdogs cannot resurrect floods
    # inside the kill sweep.
    _mixed_stop_event.set()
    _stress_stop_event.set()

    # Kill hping3 on all attacker hosts concurrently; workers exit on the
    # already-set events.
    gt_cutoff = time.time()
...
# Starve zombie telemetry immediately after the kill: dead floods' priority-10 entries keep reporting pps for ~60s, so the backend re-creates quarantines. Deleting only attacker forward rules stops that feed while legit entries keep aging naturally.
    ...
    # Reap stress workers, then campaign workers, on short deadlines; stranded
    # workers cannot respawn, so long joins only burn wall time.
    deadline = time.time() + 3
    for t in _stress_threads:
        t.join(timeout=max(0.05, deadline - time.time()))
    _stress_threads.clear()
    ...
    # One parallel safety pass catches anything spawned during the join window.
    _kill_all_attackers()
    # Repeat the forward flush: qdisc-buffered packets can reinstall a
    # priority-10 rule after the first pass.
    _flush_flow_rules([10], ips=[h.IP() for h in net.hosts
                                 if int(h.name[1:]) in _ATTACKER_POOL],
                      label="attacker forwards")

    # One batched ground-truth stop, race-safe via the cutoff.
    _notify_gt_stop_all(gt_cutoff)
...
# Batched flush of every per-IP mitigation priority plus the priority-50 proto drop. reset_flow_epochs() is deliberately not called, since flushing legit forwards at stop recreates fresh-entry false positives.
    ...
    # Bounded verification: staleness/in-flight detections can re-create
    # entries after the first clear_all, so re-poll and re-clear while dirty.
...
def _flash_crowd_run_slot(host, num: int) -> None:
    ...
    if kind == "icmp_cont":
        ...
        # raw L4 TCP, continuous loop for the full flash crowd duration
...
def _send_ryu_command(payload: dict) -> None:
    # Push a controller command to Ryu via ZMQ (same channel as reset).
    import zmq as _zmq
    _ctx  = _zmq.Context.instance()
    _sock = _ctx.socket(_zmq.PUSH)
    _sock.setsockopt(_zmq.LINGER, 0)
...
def _reset_ryu_state() -> None:
    ...
    try:
        _send_ryu_command({"action": "reset"})
        info("    Ryu state cleared.\n")
    except Exception as e:
        info(f"    Ryu reset warning: {e}\n")

def _flush_proto_cache() -> None:
    # Clear per-IP protocol caches in Ryu at campaign start. A host assigned a
    # different protocol than in the previous run would otherwise keep reporting
    # the old protocol from switches that never saw its packet-in
    # (see notes/bugs/mixed-campaign-ip-proto-leak.md).
    try:
        _send_ryu_command({"action": "flush_proto_cache"})
    except Exception as e:
        info(f"    Ryu proto cache flush warning: {e}\n")
...
def _warmup_macs() -> None:
    ...
    # each legit host pings the server once in parallel, to populate ARP
...
def _print_banner(edge_switches: list) -> None:
    info("\n" + "=" * 75 + "\n")
    info("  A-DDoS Tree Topology  |  1 core + 8 edge switches  |  26 hosts + h27\n")
    info(f"  Server:   h26 ({SERVER_IP}) - whitelisted, never ML-scored\n")
    info(f"  Sinkhole: h27 ({SINKHOLE_IP}) - silent dummy, redirected uncertain traffic\n")
    info("=" * 75 + "\n")
...
if __name__ == "__main__":
    ...
    # Expose host shortcuts as module globals so py commands always see live
    # state; a copied snapshot goes stale after stop/start cycles.
    for _h in hosts:
        globals()[_h.name] = _h
